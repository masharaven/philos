...
void pthread_init(t_gen *gen)
{
	...
	i = 0;
	thread = (pthread_t *)malloc(sizeof(pthread_mutex_t) * (gen->number_of_philo + 1));
	philo = (philo_t *)malloc(sizeof(philo_t) * gen->number_of_philo);
	gen->mutex = (pthread_mutex_t *)malloc(sizeof(pthread_mutex_t) * (gen->number_of_philo + 1));
	gen->dead_flag = 0;
